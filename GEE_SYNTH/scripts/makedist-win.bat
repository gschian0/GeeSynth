...
if %DEMO_ARG% == "demo" (
  echo Making GEE_SYNTH Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making GEE_SYNTH Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild GEE_SYNTH-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild GEE_SYNTH.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild GEE_SYNTH.sln /t:GEE_SYNTH-app;GEE_SYNTH-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\GEE_SYNTH.aaxplugin\Contents\Win32\GEE_SYNTH.aaxplugin --out .\build-win\aax\bin\GEE_SYNTH.aaxplugin\Contents\Win32\GEE_SYNTH.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\GEE_SYNTH.aaxplugin\Contents\x64\GEE_SYNTH.aaxplugin --out .\build-win\aax\bin\GEE_SYNTH.aaxplugin\Contents\x64\GEE_SYNTH.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\GEE_SYNTH.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\GEE_SYNTH.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "GEE_SYNTH Installer" ".\installer\GEE_SYNTH Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\GEE_SYNTH Installer.exe" ".\installer\GEE_SYNTH Demo Installer.exe"
  REM -del ".\installer\GEE_SYNTH Installer.exe"
  REM -)
