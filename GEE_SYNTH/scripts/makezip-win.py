...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\GEE_SYNTH Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\GEE_SYNTH Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\GEE_SYNTH manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\GEE_SYNTH.vst3\Contents\\x86_64-win\GEE_SYNTH.vst3",
      projectpath + "\\build-win\GEE_SYNTH_x64.exe"  
    ]

  zipname = "GEE_SYNTH-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\GEE_SYNTH-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\GEE_SYNTH-app_x64.pdb"  
  ]
