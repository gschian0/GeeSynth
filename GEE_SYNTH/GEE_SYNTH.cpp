...
GEE_SYNTH::GEE_SYNTH(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new IVLabelControl(logoArea, "GEE_SYNTH", DEFAULT_STYLE.WithDrawFrame(false).WithValueText(IText(50., "Logo"))));
...
#if IPLUG_DSP
void GEE_SYNTH::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void GEE_SYNTH::ProcessMidiMsg(const IMidiMsg& msg)
{
  mSynth.AddMidiMsgToQueue(msg);
}

void GEE_SYNTH::OnReset()
{
  mSynth.SetSampleRateAndBlockSize(GetSampleRate(), GetBlockSize());
}

void GEE_SYNTH::OnParamChange(int paramIdx)
{
